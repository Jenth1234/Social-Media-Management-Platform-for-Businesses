...
async function addToMailQueue(email, otp, otpType) {
    mailQueue.enqueue({ email, otp, otpType });
}
...
const HTML_TEMPLATE = (otp, otpType) => {
    let message;
...
const sendMail = async (mail, otp, otpType) => {
    
    try {
         const info = await transporter.sendMail({
        from: '"Thuc tap 2024" <nguyentpthao161203@gmail.com>', 
        to: mail, 
        subject: "Hello ✔", 
        text: "Hello world?", 
        html: HTML_TEMPLATE(otp, otpType), 
      });
    return info;
    } catch (error) {
        console.log(error);
