...
} = require("../../models/comment/validate/index");
...
class COMMENT_CONTROLLER {
  createComment = async (req, res) => {
    try {
      ...
      const { error } = createCommentValidate.validate(payload, {
        abortEarly: false,
      });
      if (error) {
  ...
  getCommentWithUserInfo = async (req, res) => {
    try {
      ...
      const commentsWithUserInfo = await commentService.getCommentWithUserInfo(
        page,
        limit
      );
  ...
  updateComment = async (req, res) => {
    try {
      ...
      const { error } = updateCommentValidate.validate(
        { CONTENT },
        { abortEarly: false }
      );
      if (error) {
      ...
      const updatedComment = await commentService.updateCommentContent(
        commentId,
        CONTENT
      );
  ...
  deleteComment = async (req, res) => {
    const { commentId, productId, organizationId } = req.params;
    const userId = req.user_id;
